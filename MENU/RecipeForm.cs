...
namespace MENU
{
    public partial class RecipeForm : Form
    {
        ...
        public RecipeForm()
        {
            ...
            _parser = new IngredientParserService(_productService);


            _youtube = new YouTubeService("AIzaSyCYQPqDOFD99Aven7RknPBtXFrOZm95Yfc");


            cmbProductName.DataSource = _productService.GetAll();
            cmbProductName.DisplayMember = "Name";
            cmbCategory.DataSource = CategoryService.Instance.Categories;
        }

        public RecipeForm(Recipe recipe) : this()
        {
        ...
        {
            var info = await _youtube.LoadVideoInfo(url);
            ...
            txtInstruction.Text = info.Description;

            if (info.ThumbnailPath != null)
            {
                picPhoto.Image = Image.FromFile(info.ThumbnailPath);
                _photoPath = info.ThumbnailPath;
            }
        }
